...
export default function CategoriesSection({
  ...
  const filteredSubCategories = React.useMemo(() => {
    if (!allCategories || !formData.categories) return [];
    return allCategories
      .filter((cat: any) => formData.categories.includes(cat.id))
      .flatMap((cat: any) => cat.sub_categories || []);
      ...
      {filteredSubCategories.length > 0 ? (
        <div className="space-y-4">
          <label className="text-sm font-semibold text-gray-700 block">
            الأقسام الفرعية المقترحة (بناءً على الأقسام الرئيسية المختارة)
          </label>
        ...
        <div className="text-center py-8 text-gray-400 border border-dashed border-gray-200 rounded-xl">
          قم باختيار قسم رئيسي لعرض الأقسام الفرعية المتاحة
        </div>
      )}
    </div>
  );
